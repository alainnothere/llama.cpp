...
ggml_tensor * llama_model_qwen4exp::graph::build_attn_qsa(
        ...
        float                     kq_scale,
        int                       il) {
    // K and V go through the same Hadamard rotation as the dense path: it flattens the stored
    // distribution for quantized caches and cancels out of the scores because the matrix is
    // orthogonal, so (qH)(kH)^T == qk^T. The attention output is rotated back below.
    if (inp->self_k_rot) {
